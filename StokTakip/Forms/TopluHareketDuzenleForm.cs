...
public class TopluHareketDuzenleForm : Form
{
    ...
    public TopluHareketDuzenleForm(List<StokHareketi> seciliHareketler)
    {
        ...
        int y = 75; // Starting Y coordinate moved down to prevent overlap with lblInfo
        ...
        pnlM.Controls.Add(chkTeslim); pnlM.Controls.Add(txtTeslim);
        y += 45;
        ...
        cmbDept.Items.Add(""); foreach (var d in Program.DB!.DepartmanlariGetir()) cmbDept.Items.Add(d);
        pnlM.Controls.Add(chkDept); pnlM.Controls.Add(cmbDept);
        y += 45;

        // Tarih
        chkTarih = MakeCheck(L("update_date"), y);
        y += 25;
        dtpTarih = new DateTimePicker { Left = 50, Top = y, Width = 340, Format = DateTimePickerFormat.Custom, CustomFormat = "dd.MM.yyyy HH:mm", Enabled = false };
        chkTarih.CheckedChanged += (_, _) => dtpTarih.Enabled = chkTarih.Checked;
        pnlM.Controls.Add(chkTarih); pnlM.Controls.Add(dtpTarih);
        y += 45;
        ...
        pnlM.Controls.Add(chkAcik); pnlM.Controls.Add(txtAciklama);
