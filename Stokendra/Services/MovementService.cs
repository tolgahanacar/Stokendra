...
public class MovementService : IMovementService
{
    ...
    public PagedResult<StockMovement> GetPagedMovements(
        DateTime? startDate,
        DateTime? endDate,
        int? stockCardId,
        string? department,
        ...
        int pageSize)
    {
        ...
        if (!string.IsNullOrWhiteSpace(searchTerm))
        {
            ...
            data = data.Where(h =>
                (h.StockCardCode != null && h.StockCardCode.ToLowerInvariant().Contains(term)) ||
                (h.StockCardName != null && h.StockCardName.ToLowerInvariant().Contains(term)) ||
                (h.Recipient != null && h.Recipient.ToLowerInvariant().Contains(term)) ||
                (h.Department != null && h.Department.ToLowerInvariant().Contains(term)) ||
                (h.Description != null && h.Description.ToLowerInvariant().Contains(term))
            ).ToList();
        }
        ...
        return new PagedResult<StockMovement>(items, totalCount, totalPages, page);
    }
    ...
    public async Task<(int Imported, int Skipped, List<string> Warnings)> ImportMovementsFromXlsxAsync(string filePath)
    {
        return await Task.Run(() =>
        {
            ...
            var hareketler = new List<StockMovement>();
            ...
            for (int r = 2; r <= lastRow; r++)
            {
                ...
                var kart = altKartlar.Find(k => k.Code.Equals(kodNo, StringComparison.OrdinalIgnoreCase));
                ...
                hareketler.Add(new StockMovement
                {
                    StockCardId = kart.Id,
                    Type = parsed.Type.ToDbString(),
